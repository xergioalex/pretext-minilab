...
npm run og-image -- --demos 35   # Regenerate with updated demo count
```
...
**Pretext Mini-Lab** is a public demo lab for the [@chenglou/pretext](https://github.com/chenglou/pretext) text layout library. It contains **28 interactive demos** organized across 4 categories (foundational, practical, advanced, spectacular), built with Astro + Svelte 5.
...
| **Demo Catalog** | `docs/demos/README.md` | Index of all 28 demos with API usage, difficulty, and rendering approach tables |
...
- **Demo list is duplicated in 3 places** that must stay in sync:
  1. `src/lib/content/demos.ts` — metadata array (single source of truth)
  2. `src/components/Nav.astro` — navigation dropdown list
  3. `src/layouts/DemoLayout.astro` — demo switcher modal list
...
├── islands/
│   └── demos/           # 28 Svelte island components (one per demo)
├── layouts/
...
│   └── demos/           # 28 demo pages (one .astro per demo)
└── styles/
...
5. Add the slug to the demos array in `src/components/Nav.astro`
6. Add the slug to the `allDemos` array in `src/layouts/DemoLayout.astro`
...
|----------|-------------|---------|
| **DOM** | Text in HTML elements, Pretext provides dimensions | Masonry, Chat, Resize Relayout, Streaming, Heatmap, Multi-Column |
...
| **Hybrid** | Positioned divs for text + SVG/Canvas for effects | Dragon Chase, Editorial Engine, Flow Around Obstacle, Wave Distortion, Text Silhouette, Waterfall, Hourglass, Earthquake |
...
npm run og-image -- --demos 35   # Also update demo count in SVG
```
...
| `requestAnimationFrame` | 15+ animated demos | 60fps animation loops |
